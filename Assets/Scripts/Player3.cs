...
public class Player3 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        lifeText = GameObject.Find("LifePlayer3").GetComponentInChildren<Text>();
        ammoText = GameObject.Find("AmmoPlayer3").GetComponentInChildren<Text>();
    ...
    void Update()
    {
        ...
        if (seconds > 0.3f)
        {
            if ((Input.GetAxisRaw("X360_RightTrigger03")) < 0)
            {
                Shoot();
                timeRemaining = 0;
            }
    ...
    void CallAim()
    {
        if (Input.GetAxisRaw("X360_RStickX03") < 0)
        {
        ...
        if (Input.GetAxisRaw("X360_RStickY03") < 0)
        {
    ...
    void Move()
    {
        var movimento = new Vector3(Input.GetAxisRaw("X360_LStickX03"), Input.GetAxisRaw("X360_LStickY03"), 0);
    ...
    public void TakeDamage(GameObject enemy)
    {
        ...
        if (life <= 0)
        {
            ...
            if (player1.activeInHierarchy || player2.activeInHierarchy || player4.activeInHierarchy)
            {
                return;
            }
            else
